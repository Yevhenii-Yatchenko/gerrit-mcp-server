...
class TestGetFileDiff(unittest.TestCase):
    ...
    @patch("gerrit_mcp_server.main.run_curl", new_callable=AsyncMock)
    def test_get_file_diff_deleted_file(self, mock_run_curl):
        async def run_test():
            diff_json = {
                "meta_a": {"name": "old_file.py", "content_type": "text/x-python", "lines": 2},
                "change_type": "DELETED",
                "content": [
                    {"a": ["line one", "line two"]},
                ],
            ...
            result = await main.get_file_diff(
                "77777", "old_file.py", gerrit_base_url="https://my-gerrit.com"
            )

            text = result[0]["text"]
            self.assertIn("old_file.py (DELETED)", text)
    ...
    @patch("gerrit_mcp_server.main.run_curl", new_callable=AsyncMock)
    def test_get_file_diff_invalid_json(self, mock_run_curl):
        async def run_test():
            mock_run_curl.return_value = "not valid json"

            result = await main.get_file_diff(
                "88888", "file.py", gerrit_base_url="https://my-gerrit.com"
            )

            text = result[0]["text"]
            self.assertIn("Failed to parse diff response", text)
